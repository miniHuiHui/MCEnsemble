...
def distillation(
    ...
    for i, data in enumerate(dataloader):
        ...
        output_student = model.encoder(images)
        output = model.head(output_student)
        #output = model(images)
        with torch.no_grad():
            output_teacher = teachers(images)

# comment this for MSELoss
        #cosine_target = torch.ones(data[1].shape).to(device)

        #loss = alpha * criterion_distillation(output_student, output_teacher, cosine_target) + beta * criterion(output,target)
        loss = alpha * criterion_distillation(output_student, output_teacher) + beta * criterion(output,target)
